...
static int ABTX_prof_print(ABTX_prof_context context, FILE *stream, int print_mode);
...
static inline uint64_t ABTXI_prof_get_cycles(void) {
  unsigned hi, lo;
...
static double ABTXI_prof_get_time_to_sec(void) {
...
static inline void ABTXI_prof_spin_lock(ABTXI_prof_spinlock *p_lock) {
  ...
  while (__atomic_test_and_set(&p_lock->val, __ATOMIC_ACQUIRE))
    ;
#else
  while (__sync_lock_test_and_set(&p_lock->val, 1)) __sync_synchronize();
#endif
}
...
static inline void ABTXI_prof_wu_time_add(ABTXI_prof_wu_time *p_wu_time, ABTXI_PROF_T val) {
  if (ABTXI_prof_unlikely(p_wu_time->cnt == 0)) {
    p_wu_time->max_val = val;
    p_wu_time->min_val = val;
  } else {
    if (p_wu_time->max_val < val) p_wu_time->max_val = val;
    if (p_wu_time->min_val > val) p_wu_time->min_val = val;
  }
  p_wu_time->sum += val;
  p_wu_time->cnt += 1;
}

static void ABTXI_prof_wu_time_merge(ABTXI_prof_wu_time *p_dest, const ABTXI_prof_wu_time *p_src) {
  if (p_dest->cnt == 0) {
    p_dest->max_val = p_src->max_val;
    p_dest->min_val = p_src->min_val;
  } else if (p_src->cnt != 0) {
    if (p_dest->max_val < p_src->max_val) p_dest->max_val = p_src->max_val;
    if (p_dest->min_val > p_src->min_val) p_dest->min_val = p_src->min_val;
  }
  p_dest->sum += p_src->sum;
  p_dest->cnt += p_src->cnt;
}

static inline void ABTXI_prof_wu_count_add(ABTXI_prof_wu_count *p_wu_count, uint64_t val) {
  if (ABTXI_prof_unlikely(p_wu_count->cnt == 0)) {
    p_wu_count->max_val = val;
    p_wu_count->min_val = val;
  } else {
    if (p_wu_count->max_val < val) p_wu_count->max_val = val;
    if (p_wu_count->min_val > val) p_wu_count->min_val = val;
  }
  p_wu_count->sum += val;
  p_wu_count->cnt += 1;
}
...
#if ABTXI_PROF_USE_TIME_LOCAL
static inline void ABTXI_prof_wu_local_time_add(ABTXI_prof_wu_local_time *p_wu_local_time,
                                                ABTXI_PROF_LOCAL_T        val) {
  if (ABTXI_prof_unlikely(p_wu_local_time->cnt == 0)) {
    p_wu_local_time->max_val = val;
    p_wu_local_time->min_val = val;
  } else {
    if (p_wu_local_time->max_val < val) p_wu_local_time->max_val = val;
    if (p_wu_local_time->min_val > val) p_wu_local_time->min_val = val;
  }
  p_wu_local_time->sum += val;
  p_wu_local_time->cnt += 1;
}

static void ABTXI_prof_wu_local_time_merge(ABTXI_prof_wu_local_time       *p_dest,
...
static ABTXI_prof_str_mem *ABTXI_prof_str_mem_alloc(int reserved) {
  ABTXI_prof_str_mem *p_str = (ABTXI_prof_str_mem *)malloc(sizeof(ABTXI_prof_str_mem) + reserved);
...
static char *ABTXI_prof_sprintf(ABTXI_prof_str_mem *p_str, size_t max_n, const char *format, ...) {
  va_list args;
...
static void ABTXI_prof_xstream_info_alloc_thread_info(ABTXI_prof_xstream_info *p_xstream_info) {
  ...
  *(void **)p_memblock            = p_xstream_info->p_memblock_head;
  ...
  while (offset + sizeof(ABTXI_prof_thread_info) <= ABTXI_PROF_MEM_BLOCK_SIZE) {
    ABTXI_prof_thread_info *p_new = (ABTXI_prof_thread_info *)(((char *)p_memblock) + offset);
  ...
  ABTXI_prof_atomic_release_store_ptr((void **)&p_xstream_info->p_thread_all, (void *)p_head_all);
}

static inline void ABTXI_prof_init_thread_info(ABTXI_prof_thread_info *p_thread_info) {
  /* Zero clear. */
...
static inline ABTXI_prof_thread_info *ABTXI_prof_get_thread_info(
    ABTXI_prof_global *p_global, ABTXI_prof_xstream_info *p_xstream_info, ABT_thread thread) {
...
static inline void ABTXI_prof_merge_thread_data(ABTXI_prof_xstream_data      *p_xstream_data,
                                                const ABTXI_prof_thread_data *p_thread_data,
                                                ABTXI_PROF_T                  time_freed) {
  /* Update statistics (counts) */
  ABTXI_prof_wu_count_add(&p_xstream_data->wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_REVIVALS],
                          p_thread_data->num_revivals);
  ABTXI_prof_wu_count_add(&p_xstream_data->wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_YIELDS],
                          p_thread_data->num_yields);
  ABTXI_prof_wu_count_add(&p_xstream_data->wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_SUSPENSIONS],
                          p_thread_data->num_suspensions);
  ABTXI_prof_wu_count_add(
      &p_xstream_data->wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_XSTREAM_CHANGES],
  ...
  if (ABTXI_prof_likely(time_elapsed != ABTXI_PROF_T_ZERO)) {
    ABTXI_prof_wu_time_add(&p_xstream_data->wu_times[ABTXI_PROF_WU_TIME_THREAD_ELAPSED],
                           time_elapsed);
  }
  if (ABTXI_prof_likely(time_created != ABTXI_PROF_T_ZERO && time_first_run != ABTXI_PROF_T_ZERO)) {
    ABTXI_prof_wu_time_add(&p_xstream_data->wu_times[ABTXI_PROF_WU_TIME_THREAD_CREATE_FIRST_RUN],
  ...
  if (ABTXI_prof_likely(time_first_run != ABTXI_PROF_T_ZERO &&
    ...
    ABTXI_prof_wu_time_add(
        &p_xstream_data->wu_times[ABTXI_PROF_WU_TIME_THREAD_FIRST_RUN_LAST_FINISH],
        time_last_finish - time_first_run);
  }
  if (ABTXI_prof_likely(time_created != ABTXI_PROF_T_ZERO &&
                        time_last_finish != ABTXI_PROF_T_ZERO)) {
    ABTXI_prof_wu_time_add(&p_xstream_data->wu_times[ABTXI_PROF_WU_TIME_THREAD_CREATE_LAST_FINISH],
                           time_last_finish - time_created);
  }
  if (ABTXI_prof_likely(time_created != ABTXI_PROF_T_ZERO && time_freed != ABTXI_PROF_T_ZERO)) {
    ABTXI_prof_wu_time_add(&p_xstream_data->wu_times[ABTXI_PROF_WU_TIME_THREAD_CREATE_FREE],
  ...
  if (ABTXI_prof_likely(time_elapsed_local != ABTXI_PROF_LOCAL_T_ZERO)) {
    ABTXI_prof_wu_local_time_add(
        &p_xstream_data->wu_local_times[ABTXI_PROF_WU_LOCAL_TIME_THREAD_ELAPSED],
        time_elapsed_local);
  }
#endif
}

static inline void ABTXI_prof_release_thread_info(ABTXI_prof_xstream_info *p_xstream_info,
                                                  ABTXI_prof_thread_info  *p_thread_info,
                                                  ABTXI_PROF_T             time_freed) {
  /* Update statistics */
  ABTXI_prof_merge_thread_data(&p_xstream_info->d, &p_thread_info->d, time_freed);
...
static void ABTXI_prof_init_xstream_info(ABTXI_prof_global       *p_global,
...
static void ABTXI_prof_reset_xstream_info(ABTXI_prof_xstream_info *p_xstream_info) {
  /* Basically zero clear. */
...
static void ABTXI_prof_destroy_xstream_info(ABTXI_prof_xstream_info *p_xstream_info) {
...
static inline ABTXI_prof_xstream_info *ABTXI_prof_get_xstream_info(ABTXI_prof_global *p_global,
                                                                   ABT_xstream        xstream) {
  if (ABTXI_prof_unlikely(xstream == ABT_XSTREAM_NULL)) {
    ABTXI_prof_xstream_info *p_xstream_info =
        (ABTXI_prof_xstream_info *)pthread_getspecific(p_global->xstream_info_key);
    if (ABTXI_prof_unlikely(!p_xstream_info)) {
      /* The first time of creation. */
      p_xstream_info = (ABTXI_prof_xstream_info *)malloc(sizeof(ABTXI_prof_xstream_info));
      ABTXI_prof_init_xstream_info(p_global, p_xstream_info, -1);
      int ret = pthread_setspecific(p_global->xstream_info_key, (void *)p_xstream_info);
      assert(ret == 0);
    ...
    if (ABTXI_prof_unlikely(ABTXI_prof_atomic_acquire_load_int(&p_global->len_p_xstreams) <=
                            rank)) {
      /* Extend the array without synchronizing readers. */
      ABTXI_prof_spin_lock(&p_global->xstreams_lock);
      int len_p_xstreams = ABTXI_prof_atomic_acquire_load_int(&p_global->len_p_xstreams);
      if (len_p_xstreams <= rank) {
        int new_len_p_xstreams =
            ABTXI_prof_max(ABTXI_PROF_DEFAULT_NUM_XSTREAMS, len_p_xstreams * 2);
        size_t size_mem_p_xstreams_p_next = sizeof(void *);
        size_t size_p_xstreams            = sizeof(ABTXI_prof_xstream_info *) * new_len_p_xstreams;
        void  *p_new_mem                  = calloc(1, size_mem_p_xstreams_p_next + size_p_xstreams);
        /* Add this new memory */
        *(void **)p_new_mem      = p_global->mem_p_xstreams;
        p_global->mem_p_xstreams = p_new_mem;
        /* Copy the current data. */
        ABTXI_prof_xstream_info **p_new_xstreams =
            (ABTXI_prof_xstream_info **)(((char *)p_new_mem) + size_mem_p_xstreams_p_next);
        memcpy(p_new_xstreams, p_global->p_xstreams,
               sizeof(ABTXI_prof_xstream_info *) * len_p_xstreams);
        ABTXI_prof_atomic_relaxed_store_ptr((void **)&p_global->p_xstreams, p_new_xstreams);
        ABTXI_prof_atomic_release_store_int(&p_global->len_p_xstreams, new_len_p_xstreams);
      } else {
    ...
    ABTXI_prof_xstream_info *p_xstream_info =
        (ABTXI_prof_xstream_info *)ABTXI_prof_atomic_relaxed_load_ptr((void **)&p_xstreams[rank]);
...
static inline int ABTXI_prof_xstream_info_get_depth_thread(ABTXI_prof_xstream_info *p_xstream_info,
                                                           uint64_t event, ABT_tool_context context,
                                                           int force_update) {
  int cur_depth;
  if (force_update || ABTXI_prof_unlikely(((cur_depth = p_xstream_info->d.cur_depth)) == 0)) {
    int stack_depth = 0;
    ABT_tool_query_thread(context, event, ABT_TOOL_QUERY_KIND_STACK_DEPTH, &stack_depth);
...
static ABTXI_prof_always_inline void ABTXI_prof_thread_callback_impl(
    ABT_thread thread, ABT_xstream xstream, uint64_t event, ABT_tool_context context,
    void *user_arg, int prof_mode) {
  ABTXI_prof_global       *p_global       = (ABTXI_prof_global *)user_arg;
  ABTXI_prof_xstream_info *p_xstream_info = ABTXI_prof_get_xstream_info(p_global, xstream);

  /* Enter the dirty phase.  This is needed to avoid the print function
   * reading premature values. */
  ABTXI_prof_atomic_release_store_int(&p_xstream_info->tag, p_xstream_info->tag + 1);

  ABTXI_prof_thread_info *p_thread_info;
  if (prof_mode == ABTX_PROF_MODE_DETAILED) {
    p_thread_info = ABTXI_prof_get_thread_info(p_global, p_xstream_info, thread);
  }
  switch (event) {
    ...
    case ABT_TOOL_EVENT_THREAD_FREE: {
      ...
      if (prof_mode == ABTX_PROF_MODE_DETAILED) {
        ABTXI_PROF_T cur_time = ABTXI_prof_get_time();
        ABTXI_prof_release_thread_info(p_xstream_info, p_thread_info, cur_time);
      }
      break;
    }
    ...
    case ABT_TOOL_EVENT_THREAD_RUN: {
      ...
      if (ABTXI_prof_unlikely(p_xstream_info->d.time_first_run_local == ABTXI_PROF_LOCAL_T_ZERO)) {
        p_xstream_info->d.time_first_run_local = cur_time_local;
      }
#endif

      int depth = ABTXI_prof_xstream_info_get_depth_thread(p_xstream_info, event, context, 1);
    ...
    case ABT_TOOL_EVENT_THREAD_FINISH: {
      ...
      int depth = ABTXI_prof_xstream_info_get_depth_thread(p_xstream_info, event, context, 0);
      if (depth < ABTXI_PROF_MAX_DEPTH) {
#if !ABTXI_PROF_USE_TIME_LOCAL
        ABTXI_PROF_T xstream_time_last_run = p_xstream_info->d.times_last_run[depth];
        if (ABTXI_prof_unlikely(xstream_time_last_run == ABTXI_PROF_T_ZERO)) {
          /* Adjustment */
          xstream_time_last_run                   = p_global->start_prof_time;
          p_xstream_info->d.times_last_run[depth] = xstream_time_last_run;
        }
        p_xstream_info->d.times_elapsed[depth] += cur_time - xstream_time_last_run;
        p_xstream_info->d.times_last_run[depth] = ABTXI_PROF_TIME_LAST_RUN_INVALID;
#else
        ABTXI_PROF_LOCAL_T xstream_time_last_run_local =
            p_xstream_info->d.times_last_run_local[depth];
        if (ABTXI_prof_unlikely(xstream_time_last_run_local == ABTXI_PROF_LOCAL_T_ZERO)) {
          /* Adjustment */
        ...
        p_xstream_info->d.times_last_run_local[depth] = ABTXI_PROF_TIME_LAST_RUN_LOCAL_INVALID;
#endif
      }
      ...
      if (prof_mode == ABTX_PROF_MODE_DETAILED) {
        ...
        ABTXI_PROF_T cur_time = ABTXI_prof_get_time();
#endif
        ABTXI_PROF_T thread_time_last_run = p_thread_info->d.time_last_run;
        if (ABTXI_prof_unlikely(thread_time_last_run == ABTXI_PROF_T_ZERO)) {
          /* Adjustment */
          thread_time_last_run           = p_global->start_prof_time;
          p_thread_info->d.time_last_run = thread_time_last_run;
        }
        p_thread_info->d.time_elapsed += cur_time - thread_time_last_run;
#if ABTXI_PROF_USE_TIME_LOCAL
        ABTXI_PROF_LOCAL_T thread_time_last_run_local = p_thread_info->d.time_last_run_local;
        if (ABTXI_prof_unlikely(thread_time_last_run_local == ABTXI_PROF_LOCAL_T_ZERO)) {
          /* Adjustment */
          thread_time_last_run_local           = cur_time_local;
          p_thread_info->d.time_last_run_local = cur_time_local;
        }
        p_thread_info->d.time_elapsed_local += cur_time_local - thread_time_last_run_local;
#endif
...
static void ABTXI_prof_thread_callback_basic(ABT_thread thread, ABT_xstream xstream, uint64_t event,
                                             ABT_tool_context context, void *user_arg) {
  ABTXI_prof_thread_callback_impl(thread, xstream, event, context, user_arg, ABTX_PROF_MODE_BASIC);
}

static void ABTXI_prof_thread_callback_detailed(ABT_thread thread, ABT_xstream xstream,
                                                uint64_t event, ABT_tool_context context,
                                                void *user_arg) {
...
static void ABTXI_prof_print_table_dsv(ABTXI_prof_data_table *p_table, const char *delimiter,
                                       FILE *stream) {
  int col, row;
...
static int ABTXI_prof_ftos_fancy(char *str, double val, int digit) {
  ...
  if (digit <= -10) {
    ...
    return sprintf(str, "%.*fe%+2d", val_accuracy - 1, val / ABTXI_prof_pow(10.0, digit), digit);
...
static void ABTXI_prof_print_table_fancy(ABTXI_prof_data_table *p_table, FILE *stream,
                                         int same_digit_from) {
  int  col, row;
...
static void ABTXI_prof_merge_xstream_info(const ABTXI_prof_global       *p_global,
  ...
  while (p_thread_head) {
    ABTXI_prof_merge_thread_data(p_out, &p_thread_head->d, ABTXI_PROF_T_ZERO);
  ...
  if (p_xstream_info->rank != -1) {
    ...
    if (p_out->cur_depth == 0) {
      ...
      for (i = 0; i < ABTXI_PROF_MAX_DEPTH; i++) {
        if (times_last_run[i] == ABTXI_PROF_T_ZERO && i < p_xstream_info->d.cur_depth) {
          ...
          times_elapsed[i] += stop_prof_time - start_prof_time;
                   ...
                   times_last_run[i] != ABTXI_PROF_TIME_LAST_RUN_INVALID) {
  ...
  for (i = 0; i < ABTXI_PROF_MAX_DEPTH; i++) p_out->times_last_run[i] += times_last_run[i];
  for (i = 0; i < ABTXI_PROF_MAX_DEPTH; i++) p_out->times_elapsed[i] += times_elapsed[i];
#else
  ...
  if (p_xstream_info->rank != -1) {
    ...
    if (p_xstream_info->d.time_first_run_local == ABTXI_PROF_LOCAL_T_ZERO) {
      ...
      for (i = 0; i < ABTXI_PROF_MAX_DEPTH; i++) {
        if (times_last_run_local[i] == ABTXI_PROF_LOCAL_T_ZERO && i < p_xstream_info->d.cur_depth) {
          /* It means that this depth has not been executed, but the
           * deeper level has been executed, implying this depth was
           * active during this time frame. */
          times_elapsed_local[i] =
              p_xstream_info->d.time_last_run_local - p_xstream_info->d.time_first_run_local;
        } else if (times_last_run_local[i] != ABTXI_PROF_LOCAL_T_ZERO &&
                   times_last_run_local[i] != ABTXI_PROF_TIME_LAST_RUN_LOCAL_INVALID) {
          /* It means that the last thread is still running on
           * this xstream.  Let's add it. */
          times_elapsed_local[i] += p_xstream_info->d.time_last_run_local - times_last_run_local[i];
        } else {
          /* times_elapsed should be correct. */
        }
      }
  ...
  for (i = 0; i < ABTXI_PROF_WU_TIME_END_; i++)
    ABTXI_prof_wu_time_merge(&p_out->wu_times[i], &p_xstream_info->d.wu_times[i]);
  for (i = 0; i < ABTXI_PROF_WU_COUNT_END_; i++)
    ABTXI_prof_wu_count_merge(&p_out->wu_counts[i], &p_xstream_info->d.wu_counts[i]);
#if ABTXI_PROF_USE_TIME_LOCAL
  for (i = 0; i < ABTXI_PROF_WU_LOCAL_TIME_END_; i++)
    ABTXI_prof_wu_local_time_merge(&p_out->wu_local_times[i], &p_xstream_info->d.wu_local_times[i]);
#endif
}

static void ABTXI_prof_print_xstream_info(ABTXI_prof_global *p_global, FILE *stream,
                                          int print_mode) {
  int i, j, rank;
  ...
  do {
    ...
    for (rank = 0; rank < p_global->len_p_xstreams; rank++) {
      if (p_global->p_xstreams[rank]) {
        if (num_ranks == len_ranks) {
          len_ranks = ABTXI_prof_max(len_ranks * 2, ABTXI_PROF_DEFAULT_NUM_XSTREAMS);
    ...
    ABTXI_prof_xstream_info  *p_xstream_info_head = p_global->p_xstream_info_head;
    while (p_xstream_info_head) {
      if (p_xstream_info_head->rank == -1) {
        ...
        if (num_external_threads == len_external_threads) {
          len_external_threads = ABTXI_prof_max(len_external_threads * 2, 16);
          external_threads     = (ABTXI_prof_xstream_info **)realloc(
              external_threads, sizeof(ABTXI_prof_xstream_info *) * len_external_threads);
        }
    ...
    /* Record all the tags. */
    int *prev_tags = (int *)malloc(sizeof(int) * (num_ranks + num_external_threads));
    for (i = 0; i < num_ranks; i++)
      prev_tags[i] = ABTXI_prof_atomic_acquire_load_int(&p_global->p_xstreams[ranks[i]]->tag);
    for (i = 0; i < num_external_threads; i++)
      prev_tags[num_ranks + i] = ABTXI_prof_atomic_acquire_load_int(&external_threads[i]->tag);

    /* Read all the data.  The last one is for external threads. */
    summaries = (ABTXI_prof_xstream_data *)calloc(num_ranks + 1, sizeof(ABTXI_prof_xstream_data));
    for (i = 0; i < num_ranks; i++) {
      ABTXI_prof_merge_xstream_info(p_global, p_global->p_xstreams[ranks[i]], &summaries[i]);
    }
    for (i = 0; i < num_external_threads; i++) {
      ABTXI_prof_merge_xstream_info(p_global, external_threads[i], &summaries[num_ranks + 1]);
    }
  ...
  ABTXI_prof_wu_count wu_counts[ABTXI_PROF_WU_COUNT_END_];
  memset(&wu_counts, 0, sizeof(ABTXI_prof_wu_count) * ABTXI_PROF_WU_COUNT_END_);
#if ABTXI_PROF_USE_TIME_LOCAL
  ABTXI_prof_wu_local_time wu_local_times[ABTXI_PROF_WU_LOCAL_TIME_END_];
  memset(&wu_local_times, 0, sizeof(ABTXI_prof_wu_local_time) * ABTXI_PROF_WU_LOCAL_TIME_END_);
#endif
  for (i = 0; i < num_ranks; i++) {
    ...
    for (j = 0; j < ABTXI_PROF_WU_COUNT_END_; j++)
      ABTXI_prof_wu_count_merge(&wu_counts[j], &summaries[i].wu_counts[j]);
#if ABTXI_PROF_USE_TIME_LOCAL
    for (j = 0; j < ABTXI_PROF_WU_LOCAL_TIME_END_; j++)
      ABTXI_prof_wu_local_time_merge(&wu_local_times[j], &summaries[i].wu_local_times[j]);
#endif
  }
  ...
  if (print_mode & ABTX_PRINT_MODE_RAW) {
    {
      ...
      table.values       = (double *)malloc(sizeof(double) * table.num_columns * table.num_rows);
      table.column_names = (const char **)malloc(sizeof(char *) * table.num_columns);
      table.row_names    = (const char **)malloc(sizeof(char *) * table.num_rows);
      /* Set the column names. */
      for (i = 0; i < num_ranks; i++)
        table.column_names[i] = ABTXI_prof_sprintf(p_str, 1024, "ES-%d", ranks[i]);
      table.column_names[num_ranks] = "Ext";
      /* Set the row names */
      for (i = 0; i < ABTXI_PROF_EVENT_END_; i++)
        table.row_names[i] =
            ABTXI_prof_sprintf(p_str, 1024, "# of events of %s", ABTXI_get_prof_event_name(i));
#if !ABTXI_PROF_USE_TIME_LOCAL
      for (i = 0; i < ABTXI_PROF_MAX_DEPTH; i++)
        table.row_names[ABTXI_PROF_EVENT_END_ + i] = ABTXI_prof_sprintf(
            p_str, 1024, "Level %d elapsed (approx.) [%s]", i, ABTXI_PROF_T_STRING);
#else
      for (i = 0; i < ABTXI_PROF_MAX_DEPTH; i++)
        table.row_names[ABTXI_PROF_EVENT_END_ + i] =
            ABTXI_prof_sprintf(p_str, 1024, "Level %d elapsed [%s]", i, ABTXI_PROF_LOCAL_T_STRING);
#endif
      /* Set the data. */
      for (i = 0; i < num_ranks + 1; i++) {
        for (j = 0; j < ABTXI_PROF_EVENT_END_; j++) {
          table.values[j * table.num_columns + i] = summaries[i].num_events[j];
        }
        for (j = 0; j < ABTXI_PROF_MAX_DEPTH; j++) {
          int row = j + ABTXI_PROF_EVENT_END_;
#if !ABTXI_PROF_USE_TIME_LOCAL
          table.values[row * table.num_columns + i] = summaries[i].times_elapsed[j];
#else
          table.values[row * table.num_columns + i] = summaries[i].times_elapsed_local[j];
#endif
        }
      }
    ...
    if (p_global->prof_mode == ABTX_PROF_MODE_DETAILED) {
...
#endif
      table.values       = (double *)malloc(sizeof(double) * 4 * table.num_rows);
      ...
      for (i = 0; i < ABTXI_PROF_WU_COUNT_END_; i++, row++)
        table.row_names[row] =
            ABTXI_prof_sprintf(p_str, 1024, "# of events of %s", ABTXI_get_prof_wu_count_name(i));
      /* Set the data. */
      row = 0;
      for (i = 0; i < ABTXI_PROF_WU_TIME_END_; i++, row++) {
        table.values[row * 4 + 0] = wu_times[i].sum;
        table.values[row * 4 + 1] = ABTXI_prof_div_s(wu_times[i].sum, wu_times[i].cnt);
      ...
      for (i = 0; i < ABTXI_PROF_WU_LOCAL_TIME_END_; i++, row++) {
        ...
        table.values[row * 4 + 1] = ABTXI_prof_div_s(wu_local_times[i].sum, wu_local_times[i].cnt);
      ...
      for (i = 0; i < ABTXI_PROF_WU_COUNT_END_; i++, row++) {
        ...
        table.values[row * 4 + 1] = ABTXI_prof_div_s((double)wu_counts[i].sum, wu_counts[i].cnt);
    ...
    const double elapsed_time = (p_global->stop_prof_time - p_global->start_prof_time) * to_sec;
    {
      int                   row = -1;
      ABTXI_prof_data_table table;
      ABTXI_prof_str_mem   *p_str = ABTXI_prof_str_mem_alloc(4096);
      ...
      table.num_rows              = 5;
      table.values       = (double *)calloc(table.num_columns * table.num_rows, sizeof(double));
      table.column_names = (const char **)malloc(sizeof(char *) * table.num_columns);
      table.row_names    = (const char **)malloc(sizeof(char *) * table.num_rows);
      /* Set the column names. */
      table.column_names[0] = "Average";
      for (i = 0; i < num_ranks; i++)
        table.column_names[i + 1] = ABTXI_prof_sprintf(p_str, 1024, "ES-%d", ranks[i]);
      ...
      for (i = 0; i < num_ranks; i++) {
        ...
        int num_finishes = summaries[i].num_events[ABTXI_PROF_EVENT_THREAD_FINISH];
#if !ABTXI_PROF_USE_TIME_LOCAL
        double granularity = ABTXI_prof_div_s(summaries[i].times_elapsed[1] * to_sec, num_finishes);
#else
        double granularity = ABTXI_prof_div_s(summaries[i].times_elapsed_local[1], num_finishes);
#endif
        table.values[row * table.num_columns] += granularity / num_ranks;
      ...
      for (i = 0; i < num_ranks; i++) {
#if !ABTXI_PROF_USE_TIME_LOCAL
        double ratio = summaries[i].times_elapsed[1] * to_sec / elapsed_time * 100.0;
#else
        double ratio =
            summaries[i].times_elapsed_local[1] / summaries[i].times_elapsed_local[0] * 100.0;
#endif
      ...
      for (i = 0; i < num_ranks; i++) {
        double nyields = summaries[i].num_events[ABTXI_PROF_EVENT_THREAD_YIELD] / elapsed_time;
      ...
      for (i = 0; i < num_ranks; i++) {
        double ncreates = summaries[i].num_events[ABTXI_PROF_EVENT_THREAD_SUSPEND] / elapsed_time;
      ...
      row                = -1;
      table.num_columns  = num_ranks + 2; /* Sum / (Ranks ...) / External */
      table.num_rows     = 2;
      table.values       = (double *)calloc(table.num_columns * table.num_rows, sizeof(double));
      table.column_names = (const char **)malloc(sizeof(char *) * table.num_columns);
      table.row_names    = (const char **)malloc(sizeof(char *) * table.num_rows);
      ...
      for (i = 0; i < num_ranks + 1; i++) {
        double ncreates = (summaries[i].num_events[ABTXI_PROF_EVENT_THREAD_CREATE]);
        table.values[row * table.num_columns] += ncreates;
        table.values[row * table.num_columns + i + 1] = ncreates;
      }
      table.row_names[++row] = "# of created ULTs [/s]";
      for (i = 0; i < num_ranks + 2; i++) {
        double ncreates_throughput = table.values[(row - 1) * table.num_columns + i] / elapsed_time;
    ...
    if (p_global->prof_mode == ABTX_PROF_MODE_DETAILED) {
      ...
      memcpy(&t_elapsed, &wu_times[ABTXI_PROF_WU_TIME_THREAD_ELAPSED], sizeof(ABTXI_prof_wu_time));
      table.values[row * 3 + 0] = ABTXI_prof_div_s(t_elapsed.sum, t_elapsed.cnt) * to_sec;
      table.values[row * 3 + 1] = t_elapsed.min_val * to_sec;
      table.values[row * 3 + 2] = t_elapsed.max_val * to_sec;
#if ABTXI_PROF_USE_TIME_LOCAL
      table.row_names[++row] =
          "ULT granularity (execution time "
          "- ES unscheduled time) [s]";
      ABTXI_prof_wu_local_time t_elapsed_local;
      memcpy(&t_elapsed_local, &wu_local_times[ABTXI_PROF_WU_LOCAL_TIME_THREAD_ELAPSED],
             sizeof(ABTXI_prof_wu_local_time));
      table.values[row * 3 + 0] = ABTXI_prof_div_s(t_elapsed_local.sum, t_elapsed_local.cnt);
      ...
      table.values[row * 3 + 0] =
          ABTXI_prof_div_s((double)wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_YIELDS].sum,
                           wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_YIELDS].cnt);
      table.values[row * 3 + 1] = wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_YIELDS].min_val;
      table.values[row * 3 + 2] = wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_YIELDS].max_val;
      table.row_names[++row]    = "# of suspend events per ULT";
      table.values[row * 3 + 0] =
          ABTXI_prof_div_s((double)wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_SUSPENSIONS].sum,
                           wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_SUSPENSIONS].cnt);
      table.values[row * 3 + 1] = wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_SUSPENSIONS].min_val;
      table.values[row * 3 + 2] = wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_SUSPENSIONS].max_val;
      table.row_names[++row]    = "# of execution stream changes per ULT";
      table.values[row * 3 + 0] =
          ABTXI_prof_div_s((double)wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_XSTREAM_CHANGES].sum,
                           wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_XSTREAM_CHANGES].cnt);
      table.values[row * 3 + 1] = wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_XSTREAM_CHANGES].min_val;
      table.values[row * 3 + 2] = wu_counts[ABTXI_PROF_WU_COUNT_THREAD_NUM_XSTREAM_CHANGES].max_val;
      table.row_names[++row] =
          "Active time per ULT (last finish "
          "time - first execution time) [s]";
      ABTXI_prof_wu_time t_active;
      memcpy(&t_active, &wu_times[ABTXI_PROF_WU_TIME_THREAD_FIRST_RUN_LAST_FINISH],
             sizeof(ABTXI_prof_wu_time));
      table.values[row * 3 + 0] = ABTXI_prof_div_s(t_active.sum, t_active.cnt) * to_sec;
      table.values[row * 3 + 1] = t_active.min_val * to_sec;
      table.values[row * 3 + 2] = t_active.max_val * to_sec;
      table.row_names[++row] =
          "Execution delay per ULT (first "
          "execution time - creation time) [s]";
      ABTXI_prof_wu_time t_delay;
      memcpy(&t_delay, &wu_times[ABTXI_PROF_WU_TIME_THREAD_CREATE_FIRST_RUN],
             sizeof(ABTXI_prof_wu_time));
      table.values[row * 3 + 0] = ABTXI_prof_div_s(t_delay.sum, t_delay.cnt) * to_sec;
      table.values[row * 3 + 1] = t_delay.min_val * to_sec;
      table.values[row * 3 + 2] = t_delay.max_val * to_sec;
      table.row_names[++row] =
          "Completion time per ULT (last "
          "finish time - creation time) [s]";
      ABTXI_prof_wu_time t_comp;
      memcpy(&t_comp, &wu_times[ABTXI_PROF_WU_TIME_THREAD_CREATE_LAST_FINISH],
             sizeof(ABTXI_prof_wu_time));
      table.values[row * 3 + 0] = ABTXI_prof_div_s(t_comp.sum, t_comp.cnt) * to_sec;
      table.values[row * 3 + 1] = t_comp.min_val * to_sec;
      table.values[row * 3 + 2] = t_comp.max_val * to_sec;
      table.row_names[++row]    = "Lifetime per ULT (free time - create time) [s]";
      ABTXI_prof_wu_time t_life;
      memcpy(&t_life, &wu_times[ABTXI_PROF_WU_TIME_THREAD_CREATE_FREE], sizeof(ABTXI_prof_wu_time));
      table.values[row * 3 + 0] = ABTXI_prof_div_s(t_life.sum, t_life.cnt) * to_sec;
...
static inline int ABTX_prof_init(ABTX_prof_context *p_new_context) {
  ...
  if (tool_enabled != ABT_TRUE) {
    fprintf(stderr,
            "[ABTX_prof_init] The tool feature is disabled: "
...
static inline int ABTX_prof_start(ABTX_prof_context context, int prof_mode) {
  ...
  if (p_global->state == ABTXI_PROF_GLOBAL_STATE_RUNNING) {
    fprintf(stderr,
            "[ABTX_prof_start] ABTX_prof_start() is called twice "
  ...
  if (prof_mode == ABTX_PROF_MODE_BASIC) {
    ABT_tool_register_thread_callback(ABTXI_prof_thread_callback_basic, ABT_TOOL_EVENT_THREAD_ALL,
                                      p_global);
  } else {
...
static inline int ABTX_prof_stop(ABTX_prof_context context) {
  ...
  ABTXI_prof_global *p_global = (ABTXI_prof_global *)context;
  if (p_global->state != ABTXI_PROF_GLOBAL_STATE_RUNNING) {
    fprintf(stderr, "[ABTX_prof_stop] ABTX_prof_start() has not been called.\n");
    return ABT_ERR_OTHER;
  }
...
static inline int ABTX_prof_print(ABTX_prof_context context, FILE *stream, int print_mode) {
  if (context == NULL) {
  ...
  ABTXI_prof_global *p_global = (ABTXI_prof_global *)context;
  if (p_global->state != ABTXI_PROF_GLOBAL_STATE_STOPPED) {
    fprintf(stderr, "[ABTX_prof_print] ABTX_prof_stop() has not been called.\n");
    return ABT_ERR_OTHER;
  }
...
static inline int ABTX_prof_clean(ABTX_prof_context context) {
  ...
  ABTXI_prof_global *p_global = (ABTXI_prof_global *)context;
  if (p_global->state != ABTXI_PROF_GLOBAL_STATE_STOPPED) {
    fprintf(stderr, "[ABTX_prof_clean] ABTX_prof_stop() has not been called.\n");
    return ABT_ERR_OTHER;
  }
  ABTXI_prof_xstream_info *p_cur = p_global->p_xstream_info_head;
  while (p_cur) {
...
static inline int ABTX_prof_finalize(ABTX_prof_context context) {
  ...
  if (ret != ABT_SUCCESS) {
    fprintf(stderr,
            "[ABTX_prof_finalize] Argobots is not initialized.  "
            "Call ABTX_prof_finalize() before ABT_finalize().\n");
    return ABT_ERR_UNINITIALIZED;
  }
  ABTXI_prof_global *p_global = (ABTXI_prof_global *)context;
  if (p_global->state == ABTXI_PROF_GLOBAL_STATE_RUNNING) {
    fprintf(stderr, "[ABTX_prof_finalize] The profiler is still running.\n");
    return ABT_ERR_OTHER;
  }
  /* Free xstreams. */
  ABTXI_prof_xstream_info *p_cur = p_global->p_xstream_info_head;
