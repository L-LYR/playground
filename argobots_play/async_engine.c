...
int main(int argc, char *argv[]) {
  ...
  ABT_xstream *engine_xstreams = (ABT_xstream *)malloc(sizeof(ABT_xstream) * (num_xstreams - 1));
  ABT_sched   *engine_scheds   = (ABT_sched *)malloc(sizeof(ABT_sched) * (num_xstreams - 1));
  operation_arg_t *ops = (operation_arg_t *)malloc(THREAD_POOL_SIZE * sizeof(operation_arg_t));
  ...
  ABT_pool_create_basic(g_wait_mode == 0 ? ABT_POOL_FIFO : ABT_POOL_FIFO_WAIT, ABT_POOL_ACCESS_MPMC,
                        ABT_TRUE, &engine_pool);
  ...
  for (i = 0; i < num_xstreams - 1; i++) {
    ABT_sched_create(&engine_sched_def, 1, &engine_pool, ABT_SCHED_CONFIG_NULL, &engine_scheds[i]);
  }
  ...
  for (step = 0; step < 2; step++) {
    ...
    if (num_total_ops >= n) num_total_ops = n;
    double interval_per_ops = (step == 0) ? 0.0 : (duration / num_total_ops);
    ...
    while (num_completed_ops < num_total_ops) {
      ...
      for (i = 0; i < THREAD_POOL_SIZE; i++) {
        ...
        if (!create_flag && num_created_ops < num_total_ops) {
          if (ABT_get_wtime() - start_time >= interval_per_ops * (num_created_ops + 1)) {
            create_flag = 1;
          }
        }
        ...
        if (create_flag && ops[i].thread == ABT_THREAD_NULL) {
          /* Create a new thread. */
          ops[i].size = size;
          ABT_thread_create(engine_pool, operation, &ops[i], ABT_THREAD_ATTR_NULL, &ops[i].thread);
          create_flag = 0;
          num_created_ops++;
          if (num_created_ops == num_total_ops) creation_end_time = ABT_get_wtime();
        }
      }
    }
    ...
    if (step != 0) {
      ...
      printf("[%d] approx. operation granularity = %f [us]\n", step,
             (compute_end_time - compute_start_time) / num_computes * 1.0e6);

      if (prof_init == ABT_SUCCESS && (prof_mode == 1 || prof_mode == 2)) {
        ABTX_prof_print(prof_context, stdout, ABTX_PRINT_MODE_SUMMARY | ABTX_PRINT_MODE_FANCY);
      }
    }
  }
